...
const faqList = [
  ...
  },
  {
    ...
    steps: [
      'Newbie Reward: Complete all tasks (Join Telegram, Watch Tutorial, etc.) to get a ₹300 bonus.',
...
export default function FaqPage() {
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F7FB] font-body">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b bg-white p-4 shadow-sm">
        <Button asChild variant="ghost" size="icon" className="h-9 w-9 rounded-full bg-slate-50">
          <Link href="/my">
            <ChevronLeft className="h-5 w-5 text-slate-800" />
          </Link>
        </Button>
        <h1 className="text-sm font-black text-slate-800 uppercase tracking-tight">Help & FAQ</h1>
        <div className="w-9"></div>
      </header>

      <main className="flex-grow p-4 space-y-4">
        <div className="text-center py-4">
            <div className="h-16 w-16 bg-blue-600 rounded-[22px] flex items-center justify-center mx-auto mb-3 shadow-xl shadow-blue-500/20">
                <BookOpen className="h-8 w-8 text-white" />
            </div>
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">User Guide</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Master Flex Pay Protocol</p>
        </div>

        <Accordion type="single" collapsible className="w-full space-y-3">
          {faqList.map((faq) => (
            <AccordionItem key={faq.id} value={faq.id} className="border-none rounded-[24px] bg-white shadow-sm overflow-hidden px-4">
              <AccordionTrigger className="py-5 hover:no-underline group">
                <div className="flex items-center gap-4">
                    <div className="grid h-9 w-9 place-items-center rounded-xl bg-slate-50 border border-slate-100 group-data-[state=open]:bg-blue-50 transition-colors">
                        {faq.icon}
                    </div>
                    <span className="font-black text-[13px] text-slate-700 uppercase tracking-tight">{faq.title}</span>
                </div>
              </AccordionTrigger>
                    ...
                    <div className="space-y-3">
                        {faq.steps.map((step, index) => (
                            <div key={index} className="flex gap-3">
                                <div className="h-5 w-5 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-black shrink-0 mt-0.5">
                                    {index + 1}
                                </div>
                                <p className="text-[11px] font-bold text-slate-500 leading-relaxed uppercase">{step}</p>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        <Card className="border-none bg-blue-600 text-white rounded-[24px] shadow-lg shadow-blue-500/20 mt-8">
            <CardContent className="p-6 text-center space-y-4">
                <HelpCircle className="h-10 w-10 mx-auto opacity-50" />
                <div>
                    <h3 className="text-lg font-black uppercase tracking-tight">Still Need Help?</h3>
                    <p className="text-[10px] font-bold text-blue-100/70 uppercase tracking-widest mt-1">Our team is available 24/7 on Telegram</p>
                </div>
                <Button asChild className="w-full bg-white text-blue-600 hover:bg-white/90 h-12 rounded-xl font-black uppercase tracking-widest text-xs">
        ...
        <div className="text-center py-8 opacity-20">
            ...
            <p className="text-[7px] font-black uppercase tracking-[0.4em]">Protocol Version 4.5.1</p>
        </div>
      </main>
    </div>
  );
