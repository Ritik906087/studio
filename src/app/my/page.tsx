              ...
              <div className="flex items-center gap-4">
                 <Avatar className="h-12 w-12 border-2 border-yellow-400">
                  ...
                  <AvatarFallback className="bg-yellow-400 text-yellow-900 font-bold">
                     {loading ? <Skeleton className="h-12 w-12 rounded-full" /> : (profile?.displayName?.charAt(0) || 'U')}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h2 className="text-lg font-bold">{loading ? <Skeleton className="h-5 w-24" /> : (profile?.displayName || 'User')}</h2>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer" onClick={(e) => {e.preventDefault(); profile && copyToClipboard(profile.numericId)}}>
                    {loading ? <Skeleton className="h-4 w-20 mt-1" /> : <><span>UID:{profile?.numericId || '...'}</span><Copy className="h-3 w-3" /></>}
                  </div>
                </div>
              </div>
        ...
        <GlassCard>
            <CardContent className="grid grid-cols-4 gap-y-6 gap-x-2 p-4 text-center">
                {actionItems.map(item => (
                  <Link href={item.href} key={item.label} className="flex flex-col items-center gap-2">
            ...
            <CardContent className="p-2">
                <Sheet>
                  ...
                  <SheetContent side="bottom" className="rounded-t-2xl">
                      ...
                      <div className="space-y-3">
                          <Link href="/my/new-user-rewards" className="block border p-4 rounded-lg hover:bg-secondary transition-colors">
                          ...
                          <Link href="/my/newbie-friend-rewards" className="block border p-4 rounded-lg hover:bg-secondary transition-colors">
                              <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-4">
                ...
                <Sheet>
                  ...
                  <SheetContent side="bottom" className="rounded-t-2xl">
                    ...
                    <div className="space-y-3">
                      <Link href="/my/report-problem" className="block border p-4 rounded-lg hover:bg-secondary transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                      ...
                      <Link href="/my/report-status" className="block border p-4 rounded-lg hover:bg-secondary transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                      ...
                      <Link href="/my/feedback" className="block border p-4 rounded-lg hover:bg-secondary transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
